...
# Df track: 4. Adding a column sets dataframe_changed True
#           5. Reassigning df to fewer rows sets dataframe_changed True
#           6. Code that doesn't touch df leaves dataframe_changed False
...
@pytest.fixture
def sample_df_pickle(sample_df) -> bytes:
    return pickle.dumps(sample_df)


# ── Stdout capture ────────────────────────────────────────────────────────────

def test_execute_captures_stdout(sample_df_pickle):
    # When the code is print('hello world'), result should contain "hello world" in stdout and no error.
    result = _execute_in_sandbox("print('hello world')", sample_df_pickle)
...
def test_execute_captures_single_figure_as_base64_png(sample_df_pickle):
    # When code creates one matplotlib figure, result should contain 1 figure that decodes to a valid PNG.
    code = "plt.figure()\nplt.plot([1, 2, 3], [4, 5, 6])"
    result = _execute_in_sandbox(code, sample_df_pickle)
    assert result["error"] is None
...
def test_execute_captures_multiple_figures(sample_df_pickle):
...
def test_execute_column_addition_sets_dataframe_changed(sample_df_pickle):
    # When code adds a column with df['z'] = df['x'] + df['y'],
    # result should report dataframe_changed as True.
    result = _execute_in_sandbox("df['z'] = df['x'] + df['y']", sample_df_pickle)
    assert result["error"] is None
    assert result["dataframe_changed"] is True


def test_execute_row_reduction_sets_dataframe_changed(sample_df_pickle):
    # When code reassigns df to fewer rows with df = df.head(1),
    # result should report dataframe_changed as True.
    result = _execute_in_sandbox("df = df.head(1)", sample_df_pickle)
    assert result["error"] is None
    assert result["dataframe_changed"] is True


def test_execute_no_df_touch_leaves_dataframe_unchanged(sample_df_pickle):
    # When code doesn't touch df at all (e.g. x = 1 + 1),
    # result should report dataframe_changed as False.
    result = _execute_in_sandbox("x = 1 + 1", sample_df_pickle)
    assert result["error"] is None
...
def test_execute_syntax_error_returns_error(sample_df_pickle):
    # When code has a syntax error like def foo(,
    # result should contain "SyntaxError" in the error field.
    result = _execute_in_sandbox("def foo(", sample_df_pickle)
    assert result["error"] is not None
    assert "SyntaxError" in result["error"]


def test_execute_runtime_error_returns_error(sample_df_pickle):
    # When code raises a runtime error like 1 / 0,
    # result should contain "ZeroDivisionError" in the error field.
    result = _execute_in_sandbox("1 / 0", sample_df_pickle)
    assert result["error"] is not None
...
def test_execute_blocks_import(sample_df_pickle):
    ...
    result = _execute_in_sandbox("import os", sample_df_pickle)
    assert result["error"] is not None


def test_execute_blocks_open(sample_df_pickle):
    # When code tries open('/etc/passwd'), it should be blocked and result should contain an error.
    result = _execute_in_sandbox("open('/etc/passwd')", sample_df_pickle)
    assert result["error"] is not None


def test_execute_preloaded_libraries_remain_accessible(sample_df_pickle):
...
def test_execute_timeout_kills_long_running_code(sample_df):
    ...
    result = execute_code("while True: pass", {"df": sample_df}, timeout=2)
    assert result["error"] is not None
